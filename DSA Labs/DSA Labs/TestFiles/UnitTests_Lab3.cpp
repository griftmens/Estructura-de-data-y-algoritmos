...
int UnitTests_Lab3::Battery_EraseEmpty() {
	...
	int totalDeductions = UnitTestBattery("Testing Erase on an empty list", LAB3_ERASE_EMPTY,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(LAB3_ERASE_EMPTY, totalDeductions);
...
FailResult UnitTests_Lab3::Fail_AssignmentOperator_NotEnoughMemoryAllocated() {
	...
	size_t memoryDeltaEnd = inUse;
	size_t memoryAllocated = (memoryDeltaStart - memoryDeltaEnd);
	size_t correctAmountOfMemory = sizeof(Node) * (static_cast<int64_t>(numberOfNodes2) - numberOfNodes1);
